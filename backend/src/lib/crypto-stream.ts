...
const KRAKEN_REVERSE: Record<string, string> = {};
for (const [pair, ksym] of Object.entries(KRAKEN_PAIR_MAP)) {
  if (!KRAKEN_REVERSE[ksym]) KRAKEN_REVERSE[ksym] = pair;
}

// ── Coinbase symbol map (pair → Coinbase product_id) ─────────────────────────
function toCoinbaseProduct(pair: string): string {
  return pair.replace('/USDT', '-USD').replace('/USD', '-USD').replace('/', '-')
    .replace('POL-USD', 'MATIC-USD');
}
// Reverse: Coinbase product_id → our pair format
function fromCoinbaseProduct(productId: string): string {
  return productId.replace('MATIC-USD', 'MATIC/USDT').replace('-USD', '/USDT').replace('-', '/');
}

// ── KuCoin symbol map (pair → KuCoin topic symbol) ───────────────────────────
function toKuCoinSymbol(pair: string): string {
  const base = pair.split('/')[0] ?? pair;
  const kcBase = base === 'POL' ? 'MATIC' : base;
  return `${kcBase}-USDT`;
}
function fromKuCoinSymbol(sym: string): string {
  const base = sym.replace('-USDT', '');
  return `${base}/USDT`;
}

// ── Generic stream base ───────────────────────────────────────────────────────
abstract class ExchangeStream {
  protected ws: WebSocket | null = null;
  protected reconnectTimer: ReturnType<typeof setTimeout> | null = null;
  protected reconnectDelay = 2000;
  protected shouldRun = false;
  protected symbolRefs = new Map<string, number>(); // pair → subscriber count

  addSymbols(pairs: string[]) {
    const toAdd: string[] = [];
    for (const p of pairs) {
      const prev = this.symbolRefs.get(p) ?? 0;
      this.symbolRefs.set(p, prev + 1);
      if (prev === 0) toAdd.push(p);
    }
    if (toAdd.length > 0) {
      if (!this.shouldRun) { this.shouldRun = true; this.connect(); }
      else this.subscribe(toAdd);
    }
  }

  removeSymbols(pairs: string[]) {
    const toRemove: string[] = [];
    for (const p of pairs) {
      const prev = this.symbolRefs.get(p) ?? 0;
      if (prev <= 1) { this.symbolRefs.delete(p); toRemove.push(p); }
      else this.symbolRefs.set(p, prev - 1);
    }
  ...
  protected abstract subscribe(pairs: string[]): void;
  protected abstract unsubscribe(pairs: string[]): void;
}
...
class KrakenStream extends ExchangeStream {
  ...
  private supported(pair: string) { return KRAKEN_SUPPORTED.has(pair); }
  ...
  protected connect() {
    ...
    ws.on('open', () => {
      ...
      if (syms.length > 0) this.subscribe(syms);
    });
      ...
      try {
        ...
        if (msg.channel === 'ticker' && (msg.type === 'update' || msg.type === 'snapshot') && Array.isArray(msg.data)) {
          for (const d of msg.data) {
            const krakenSym = d.symbol as string;
            const price = d.last as number;
            if (!krakenSym || !price) continue;
    ...
    ws.on('close', () => {
      this.ws = null;
      if (this.shouldRun && this.symbolRefs.size > 0) this.scheduleReconnect();
    });
    ws.on('error', () => ws.close());
  }

  protected subscribe(pairs: string[]) {
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) return;
    const krakenSyms = pairs.filter(p => KRAKEN_SUPPORTED.has(p)).map(p => KRAKEN_PAIR_MAP[p]!);
    if (krakenSyms.length === 0) return;
  ...
  protected unsubscribe(pairs: string[]) {
    if (!this.ws || this.ws.readyState !== WebSocket.OPEN) return;
    const krakenSyms = pairs.filter(p => KRAKEN_SUPPORTED.has(p)).map(p => KRAKEN_PAIR_MAP[p]!);
    if (krakenSyms.length === 0) return;
    this.ws.send(JSON.stringify({
...
class CoinbaseStream extends ExchangeStream {
  protected connect() {
    ...
    ws.on('open', () => {
      ...
      if (pairs.length > 0) this.subscribe(pairs);
    });
...
class KuCoinStream extends ExchangeStream {
  private endpoint = 'wss://ws-api.kucoin.com/endpoint';
  private token = '';
  ...
  protected async connect() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (server?.endpoint) this.endpoint = server.endpoint;
      }
    } catch {}
...
class CryptoStreamService {
  private kraken  = new KrakenStream();
  private coinbase = new CoinbaseStream();
  private kucoin  = new KuCoinStream();

  /**
   * Subscribe pairs for a given exchange.
   * Pairs not supported by the requested exchange fall through to KuCoin.
   */
  addPairs(exchange: string, pairs: string[]) {
    if (exchange === 'kraken') {
      const supported   = pairs.filter(p => KRAKEN_SUPPORTED.has(p));
      const unsupported = pairs.filter(p => !KRAKEN_SUPPORTED.has(p));
      if (supported.length > 0)   this.kraken.addSymbols(supported);
      if (unsupported.length > 0) this.kucoin.addSymbols(unsupported);
  ...
  removePairs(exchange: string, pairs: string[]) {
    if (exchange === 'kraken') {
      const supported   = pairs.filter(p => KRAKEN_SUPPORTED.has(p));
      const unsupported = pairs.filter(p => !KRAKEN_SUPPORTED.has(p));
      if (supported.length > 0)   this.kraken.removeSymbols(supported);
      if (unsupported.length > 0) this.kucoin.removeSymbols(unsupported);
    } else if (exchange === 'coinbase') {
