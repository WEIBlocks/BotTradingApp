...
export async function generateEmbedding(text: string): Promise<number[]> {
  ...
  if (!input) return fakeEmbedding('empty');
  ...
  if (env.GEMINI_API_KEY) {
    try {
      ...
      console.warn('[Embeddings] Gemini failed, falling back:', (err as Error).message);
    }
  }
  ...
  if (env.OPENAI_API_KEY) {
    try {
      ...
      console.warn('[Embeddings] OpenAI failed, falling back:', (err as Error).message);
    }
  }
