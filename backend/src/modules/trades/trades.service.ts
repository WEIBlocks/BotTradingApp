...
async function getArenaTrades(userId: string, limit: number): Promise<any[]> {
  try {
    ...
    console.warn('[Trades] Arena query failed:', (err as Error).message);
    return [];
  }
}
...
export async function getRecentTrades(userId: string, limit = 10) {
  ...
  const regularTrades = (rows as unknown as any[]).map(r => ({
...
export async function getTradeHistory(userId: string, filters: TradeHistoryFilters) {
  ...
  const paginationParams: PaginationParams = { page, limit: lim };
  ...
  if (filters.mode === 'live') conditions.push(sql`t.is_paper = false AND t.bot_subscription_id IS NOT NULL`);
  else if (filters.mode === 'shadow') conditions.push(sql`t.shadow_session_id IS NOT NULL`);
  else if (filters.is_paper !== undefined) conditions.push(sql`t.is_paper = ${filters.is_paper === 'true'}`);
  ...
  const enriched = (rows as unknown as any[]).map(r => ({
    id: r.id, symbol: r.symbol, side: r.side, amount: r.amount, price: r.price,
    totalValue: r.total_value, pnl: r.pnl, pnlPercent: r.pnl_percent,
    isPaper: r.is_paper, reasoning: r.reasoning, status: r.status,
    executedAt: r.executed_at_str, mode: r.mode ?? 'live',
  ...
  if (!filters.mode || filters.mode === 'all' || filters.mode === 'arena') {
    ...
    const allRegularRows = await db.execute(sql`
      ...
      ORDER BY t.executed_at DESC
    `);
    const allRegular = (allRegularRows as unknown as any[]).map(r => ({
      id: r.id, symbol: r.symbol, side: r.side, amount: r.amount, price: r.price,
      totalValue: r.total_value, pnl: r.pnl, pnlPercent: r.pnl_percent,
      isPaper: r.is_paper, reasoning: r.reasoning, status: r.status,
      executedAt: r.executed_at_str, mode: r.mode ?? 'live',
...
export async function getTradeSummary(userId: string) {
  ...
  const byMode: Record<string, {total: number; totalPnl: number; wins: number; totalSells: number}> = {};
  ...
  const arenaRows = await db.execute(sql`
    ...
    WHERE ars.user_id = ${userId}
  `);
  const arenaRow = (arenaRows as unknown as any[])[0];
