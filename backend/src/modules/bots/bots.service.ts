...
import { eq, and, or, sql, desc, asc, gte, inArray } from 'drizzle-orm';
...
export async function getShadowSessionConfig(userId: string, sessionId: string) {
...
export async function updateShadowSessionConfig(userId: string, sessionId: string, data: Record<string, any>) {
  const [session] = await db.select().from(shadowSessions)
    .where(and(eq(shadowSessions.id, sessionId), eq(shadowSessions.userId, userId)));
  if (!session) throw new NotFoundError('Shadow session');
...
export async function deleteBot(userId: string, botId: string) {
  ...
  if (activeSub) {
    throw new ConflictError(
      `Cannot delete: this bot is currently running in ${activeSub.mode === 'live' ? 'live' : 'paper'} mode. Stop all active subscriptions first.`,
    );
  }
  ...
  const [activeShadow] = await db
    ...
    .where(
      and(
        eq(shadowSessions.botId, botId),
        inArray(shadowSessions.status, ['running', 'paused'] as const),
      ),
    )
    .limit(1);
  ...
  if (activeShadow) {
    throw new ConflictError(
      'Cannot delete: this bot has an active shadow mode session. Stop the shadow run first.',
    );
  }
  ...
  if (activeCopy) {
    throw new ConflictError(
      'Cannot delete: this bot has active copy-trading followers. They must stop following first.',
    );
  }
  ...
  if (openPos) {
    throw new ConflictError(
      'Cannot delete: this bot has open positions. Close all positions before deleting.',
    );
  }
