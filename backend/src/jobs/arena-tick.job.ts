      ...
      try {
        ...
        for (const { gladiator, botStrategy, botPrompt, botRiskLevel, botConfig, botCategory, botName } of gladiators) {
          ...
          try {
            ...
            (state as any)._closedCount = closedPositions.length;
          } catch (err) {
          ...
          const closedCount = (state as any)._closedCount ?? state.trades;
...
async function finalizeArenaSession(sessionId: string, userId: string) {
  ...
  try {
    ...
    for (const gladiator of gladiators) {
      ...
      const arenaPositions = await db.select().from(botPositions).where(and(
        eq(botPositions.botId, gladiator.botId),
        eq(botPositions.userId, userId),
        eq(botPositions.isPaper, !isLiveMode),
        eq(botPositions.shadowSessionId, gladiator.id),
      ));
