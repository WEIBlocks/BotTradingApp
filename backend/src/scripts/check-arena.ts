...
async function checkArena() {
  ...
  for (const s of sessions) {
    ...
    for (const { g, botName, botConfig } of glads) {
      ...
      const decisions = Array.isArray(g.decisionLog) ? g.decisionLog as any[] : [];
      const cfg = (botConfig ?? {}) as any;
      console.log(`\n    Bot: ${botName}`);
      ...
      if (decisions.length > 0) {
        const nonHold = decisions.filter((d: any) => d.action !== 'HOLD');
        console.log(`      non-HOLD decisions: ${nonHold.length}`);
        if (nonHold.length > 0) console.log(`      last trade: ${JSON.stringify(nonHold[nonHold.length-1])}`);
      }
    }
