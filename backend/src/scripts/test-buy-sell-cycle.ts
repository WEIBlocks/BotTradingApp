...
function binanceSig(secret: string, query: string) {
  return crypto.createHmac('sha256', secret).update(query).digest('hex');
}

async function binanceRequest(method: 'GET' | 'POST' | 'DELETE', path: string, params: Record<string, string | number>, apiKey: string, apiSecret: string) {
  const ts = Date.now();
  const p: Record<string, string> = { ...Object.fromEntries(Object.entries(params).map(([k, v]) => [k, String(v)])), timestamp: String(ts), recvWindow: '15000' };
...
async function testBinance(apiKey: string, apiSecret: string) {
  ...
  if (!acctBefore.ok) { console.error('  ✗ Account fetch failed:', acctBefore.body); return; }
  const usdtBefore = parseFloat(acctBefore.body.balances?.find((b: any) => b.asset === 'USDT')?.free ?? '0');
  const btcBefore  = parseFloat(acctBefore.body.balances?.find((b: any) => b.asset === 'BTC')?.free ?? '0');
  ...
  const usdtAfter = parseFloat(acctAfter.body.balances?.find((b: any) => b.asset === 'USDT')?.free ?? '0');
  const btcAfter  = parseFloat(acctAfter.body.balances?.find((b: any) => b.asset === 'BTC')?.free ?? '0');
...
async function alpacaRequest(method: string, path: string, body?: object, apiKey?: string, apiSecret?: string) {
...
async function testAlpaca(apiKey: string, apiSecret: string) {
  console.log('\n════════════════════════════════════════');
