...
const OHLCV_HISTORY_KEY = (symbol) => `history:closes:${symbol.replace('/', ':')}`;
const OHLCV_CANDLES = 100;
const OHLCV_TTL = 7200; // 2 hours
async function fetchAndStoreOHLCV(symbol) {
    try {
        ...
        const closes = candles
            .map((c) => parseFloat(c[2])) // index 2 = close price
            .filter((p) => p > 0)
            .reverse();
        if (closes.length < 10)
            return;
...
async function maybeRefreshOHLCV() {
    ohlcvSyncCounter++;
    if (ohlcvSyncCounter % 5 !== 0)
        return; // every ~2.5 minutes
