...
async function pauseUserBots(userId) {
    // Pause all active/live bot subscriptions
    await db.update(botSubscriptions)
        .set({ status: 'paused', updatedAt: new Date() })
        .where(and(eq(botSubscriptions.userId, userId), eq(botSubscriptions.status, 'active')));
...
async function checkExpiredSubscriptions() {
    ...
    const expiring = await db
        ...
        .where(and(eq(userSubscriptions.status, 'active'), eq(subscriptionPlans.tier, 'pro'), lte(userSubscriptions.currentPeriodEnd, in24h)));
