            ...
            try {
                ...
                if (userConfig.autoStopLossPercent !== undefined && userConfig.autoStopLossPercent > 0 && allocatedAmount > 0) {
                    const [lossRow] = await db
                        ...
                        .from(trades)
                        .where(and(eq(trades.botSubscriptionId, subscription.id), lt(trades.pnl, '0')));
                ...
                if (!conn || conn.status === 'disconnected' || conn.status === 'error') {
                    if (!isStockBot) {
                        ...
                        const [fallbackConn] = await db
                            .select()
                            .from(exchangeConnections)
                            .where(and(eq(exchangeConnections.userId, subscription.userId), eq(exchangeConnections.assetClass, 'crypto'), eq(exchangeConnections.status, 'connected')))
                            .limit(1);
                        if (fallbackConn) {
