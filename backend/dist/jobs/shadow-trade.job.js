        ...
        for (const { session, bot } of activeSessions) {
            try {
                ...
                const [subRow] = await db
                    ...
                    .where(and(eq(botSubscriptions.userId, session.userId), eq(botSubscriptions.botId, session.botId)))
                    .limit(1);
                const userConfig = (subRow?.userConfig ?? {});
                ...
                if (newTrades > 0) {
                    ...
                    try {
                        ...
                        const closedPos = await db
                            ...
                            .from(trades)
                            .where(and(eq(trades.shadowSessionId, session.id), eq(trades.userId, session.userId), eq(trades.isPaper, true), eq(trades.side, 'SELL')));
                        let cumPnl = 0;
