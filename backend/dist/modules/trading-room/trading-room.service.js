...
export async function getMessages(limit, beforeId) {
    const conditions = beforeId
        ? lt(tradingRoomMessages.createdAt, sql `(SELECT created_at FROM trading_room_messages WHERE id = ${beforeId})`)
        : undefined;
...
async function notifyProUsers(senderId, senderName, content) {
    ...
    for (let i = 0; i < userIds.length; i += 20) {
        ...
        await Promise.all(batch.map(uid => sendNotification(uid, {
            type: 'system',
...
export async function deleteMessage(userId, messageId, role) {
    const [message] = await db
        .select()
        .from(tradingRoomMessages)
...
export async function getOnlineCount() {
    ...
    const [result] = await db
        .select({ count: sql `count(distinct ${tradingRoomMessages.userId})` })
        .from(tradingRoomMessages)
...
export async function getMembers() {
    ...
    const members = await db
        ...
        .from(users)
        .where(sql `${users.role} = 'admin' OR ${users.id} IN (
