...
export async function supportRoutes(app) {
    ...
    app.post('/support/tickets', { preHandler: [authenticate] }, async (request, reply) => {
        const { type, title, description, category } = request.body;
    ...
    app.post('/admin/support/tickets/:ticketId/messages', { preHandler: [authenticate, authorize('admin')] }, async (request, reply) => {
        ...
        await supportService.sendDirectNotification(ticket.userId, 'Support Reply', `Your ticket "${ticket.title}" has a new response.`);
        return { data: message };
    });
