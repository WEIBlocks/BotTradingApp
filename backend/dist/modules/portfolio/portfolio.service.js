...
async function resolveSandbox(userId, mode) {
    if (mode === 'live')
        return false;
    if (mode === 'testnet')
        return true;
...
export async function getConnectedModes(userId) {
    const conns = await db
...
export async function getSummary(userId, mode) {
    ...
    const [result] = await db
        .select({
        totalValue: sql `COALESCE(SUM(CASE WHEN ${exchangeAssets.amount}::numeric > 0 THEN ${exchangeAssets.valueUsd}::numeric ELSE 0 END), 0)`,
        change24h: sql `COALESCE(
    ...
    const [pnlResult] = await db
        .select({
        totalPnl: sql `COALESCE(SUM(${botPositions.pnl}::numeric), 0)`,
        totalTrades: count(),
    })
        .from(botPositions)
...
export async function getAssets(userId, mode) {
    const sandbox = await resolveSandbox(userId, mode);
...
export async function getAllocation(userId, mode) {
    ...
    const rows = await db
        ...
        symbol: exchangeAssets.symbol,
        name: exchangeAssets.name,
        totalValue: sql `SUM(CASE WHEN ${exchangeAssets.amount}::numeric > 0 THEN ${exchangeAssets.valueUsd}::numeric ELSE 0 END)`,
        totalAmount: sql `SUM(CASE WHEN ${exchangeAssets.amount}::numeric > 0 THEN ${exchangeAssets.amount}::numeric ELSE 0 END)`,
        iconColor: exchangeAssets.iconColor,
    })
        .from(exchangeAssets)
        .innerJoin(exchangeConnections, eq(exchangeAssets.exchangeConnId, exchangeConnections.id))
 ...
 */
export async function getPnlByBot(userId) {
