...
import { bots, botStatistics, botSubscriptions, shadowSessions, reviews, botVersions, creatorEarnings, botFavorites, } from '../../db/schema/bots.js';
...
export async function updateBot(userId, botId, data) {
    // Verify ownership
    ...
    if (data.priceMonthly !== undefined)
        updates.priceMonthly = String(data.priceMonthly);
    if (data.creatorFeePercent !== undefined)
        updates.creatorFeePercent = String(data.creatorFeePercent);
    // Strategy core
    ...
    if (data.risk_level !== undefined)
        updates.riskLevel = data.risk_level;
    if (data.prompt !== undefined)
        updates.prompt = data.prompt;
    // Merge config (jsonb) fields. Every "execution" / "risk" knob lives here.
    const existingConfig = existing.config ?? {};
    const configUpdates = { ...existingConfig };
    if (data.pairs !== undefined)
        configUpdates.pairs = data.pairs;
    if (data.stopLoss !== undefined)
        configUpdates.stopLoss = data.stopLoss;
    if (data.takeProfit !== undefined)
        configUpdates.takeProfit = data.takeProfit;
    if (data.maxPositionSize !== undefined)
        configUpdates.maxPositionSize = data.maxPositionSize;
    if (data.dailyLossLimit !== undefined)
        configUpdates.dailyLossLimit = data.dailyLossLimit;
    if (data.maxOpenPositions !== undefined)
        configUpdates.maxOpenPositions = data.maxOpenPositions;
    if (data.tradeDirection !== undefined)
        configUpdates.tradeDirection = data.tradeDirection;
    if (data.orderType !== undefined)
        configUpdates.orderType = data.orderType;
    if (data.tradingFrequency !== undefined)
        configUpdates.tradingFrequency = data.tradingFrequency;
    if (data.tradingSchedule !== undefined)
        configUpdates.tradingSchedule = data.tradingSchedule;
    if (data.aiMode !== undefined)
        configUpdates.aiMode = data.aiMode;
    if (data.maxHoldsBeforeAI !== undefined)
        configUpdates.maxHoldsBeforeAI = data.maxHoldsBeforeAI;
    if (data.aiConfidenceThreshold !== undefined)
        configUpdates.aiConfidenceThreshold = data.aiConfidenceThreshold;
    if (data.customEntryConditions !== undefined)
        configUpdates.customEntryConditions = data.customEntryConditions;
    if (data.customExitConditions !== undefined)
        configUpdates.customExitConditions = data.customExitConditions;
...
export async function getBotForEdit(userId, botId) {
    const [bot] = await db
        .select()
        .from(bots)
        .where(and(eq(bots.id, botId), eq(bots.creatorId, userId)));
...
export async function pauseBot(userId, botSubId) {
    const [sub] = await db
        .select()
...
export async function stopBot(userId, botSubId) {
    const [sub] = await db
        .select()
        .from(botSubscriptions)
        .where(and(eq(botSubscriptions.id, botSubId), eq(botSubscriptions.userId, userId)));
...
export async function resumeBot(userId, botSubId) {
    const [sub] = await db
        .select()
        .from(botSubscriptions)
        .where(and(eq(botSubscriptions.id, botSubId), eq(botSubscriptions.userId, userId)));
...
export async function startShadowMode(userId, botId, config) {
    // Check bot exists
    const [bot] = await db.select().from(bots).where(eq(bots.id, botId));
    if (!bot) {
        throw new NotFoundError('Bot');
    ...
    const [existingShadow] = await db
        .select()
        .from(shadowSessions)
        .where(and(eq(shadowSessions.userId, userId), eq(shadowSessions.botId, botId), eq(shadowSessions.status, 'running')))
        .limit(1);
    if (existingShadow) {
...
export async function getShadowResults(userId, sessionId) {
    const [session] = await db
        .select()
        .from(shadowSessions)
    ...
    const rawDaily = session.dailyPerformance;
    let dailyPerfArray = [];
...
export async function pauseShadowSession(userId, sessionId) {
    const [session] = await db
        .select()
        .from(shadowSessions)
        .where(and(eq(shadowSessions.id, sessionId), eq(shadowSessions.userId, userId)));
...
export async function resumeShadowSession(userId, sessionId) {
    const [session] = await db
        .select()
        .from(shadowSessions)
        .where(and(eq(shadowSessions.id, sessionId), eq(shadowSessions.userId, userId)));
...
export async function stopShadowSession(userId, sessionId) {
    const [session] = await db
        .select()
        .from(shadowSessions)
        .where(and(eq(shadowSessions.id, sessionId), eq(shadowSessions.userId, userId)));
...
export async function getUserShadowSessions(userId) {
...
export async function getUserActiveBots(userId) {
    const rows = await db
        .select({
...
export async function backtestBot(userId, botId, config) {
    const [bot] = await db.select().from(bots).where(eq(bots.id, botId));
    if (!bot)
        throw new NotFoundError('Bot');
...
export async function addReview(userId, botId, rating, text) {
...
// ─── Paper Trading ──────────────────────────────────────────────────────────
export async function getPaperTradingStatus(userId) {
    ...
    const runningSessions = await db
        ...
        .from(shadowSessions)
        .leftJoin(bots, eq(shadowSessions.botId, bots.id))
        .where(and(eq(shadowSessions.userId, userId), eq(shadowSessions.status, 'running')))
        .orderBy(desc(shadowSessions.startedAt));
    const totalVirtualBalance = runningSessions.reduce((sum, s) => sum + parseFloat(s.currentBalance ?? s.virtualBalance), 0);
...
export async function activateLiveMode(userId, botId, exchangeConnId, allocatedAmount) {
...
export async function getBotDecisions(userId, botId, limit = 50, offset = 0, mode) {
    const conditions = [
    ...
    const [stats] = await db
        .select({
        total: sql `count(*)::int`,
        totalBuys: sql `count(*) FILTER (WHERE ${botDecisions.action} = 'BUY')::int`,
        totalSells: sql `count(*) FILTER (WHERE ${botDecisions.action} = 'SELL')::int`,
        totalHolds: sql `count(*) FILTER (WHERE ${botDecisions.action} = 'HOLD')::int`,
        totalAiCalls: sql `count(*) FILTER (WHERE ${botDecisions.aiCalled} = true)::int`,
        totalTokens: sql `COALESCE(sum(${botDecisions.tokensCost}), 0)::int`,
    })
        .from(botDecisions)
...
// ─── Compare Bots ───────────────────────────────────────────────────────────
export async function compareBots(botIds) {
    if (botIds.length === 0)
        return [];
    if (botIds.length > 5)
        botIds = botIds.slice(0, 5);
    const results = [];
    for (const botId of botIds) {
        const [bot] = await db.select().from(bots).where(eq(bots.id, botId));
        const [stats] = await db.select().from(botStatistics).where(eq(botStatistics.botId, botId));
        const posCount = await db.select({ count: sql `count(*)::int` }).from(botPositions)
            .where(and(eq(botPositions.botId, botId), eq(botPositions.status, 'closed')));
        if (bot) {
            results.push({
...
// ─── Copy Trading ───────────────────────────────────────────────────────────
export async function startCopyTrading(userId, botId, allocationPercent = 100, isPaper = true) {
    const [bot] = await db.select().from(bots).where(eq(bots.id, botId));
    if (!bot)
        throw new NotFoundError('Bot');
...
export async function stopCopyTrading(userId, botId) {
    const { copyTradingSessions } = await import('../../db/schema/copy-trading');
    const [session] = await db.update(copyTradingSessions).set({
        status: 'stopped', stoppedAt: new Date(),
    }).where(and(eq(copyTradingSessions.followerId, userId), eq(copyTradingSessions.botId, botId))).returning();
    if (!session)
        throw new NotFoundError('Copy trading session');
    return session;
}
export async function getBotEquityCurve(botId, userId, days = 30) {
    ...
    const closedPositions = await db
        ...
        .from(botPositions)
        .where(and(eq(botPositions.botId, botId), eq(botPositions.userId, userId), eq(botPositions.status, 'closed'), gte(botPositions.closedAt, sql `now() - (${days} || ' days')::interval`)))
        .orderBy(asc(botPositions.closedAt));
    // Build cumulative equity curve starting from user's actual balance
    let cumPnl = 0;
    const equityData = [startingBalance]; // start at user's allocated balance
    const dates = [new Date(Date.now() - days * 86400000).toISOString()];
...
export async function getBotTradeMarkers(botId, symbol, days = 30) {
    ...
    const markers = await db
        ...
        .from(botDecisions)
        .where(and(eq(botDecisions.botId, botId), eq(botDecisions.symbol, symbol), sql `${botDecisions.action} IN ('BUY', 'SELL')`, gte(botDecisions.createdAt, sql `now() - (${days} || ' days')::interval`)))
...
export async function getPublicLiveStats(botId) {
    const [bot] = await db.select().from(bots).where(eq(bots.id, botId));
    if (!bot)
        throw new NotFoundError('Bot');
...
export async function getMyLiveStats(userId, botId) {
    // Get the user's live subscription (any status — include stopped so past stats remain)
    const [sub] = await db.select()
        .from(botSubscriptions)
        .where(and(eq(botSubscriptions.botId, botId), eq(botSubscriptions.userId, userId), eq(botSubscriptions.mode, 'live')))
    ...
    let equityCum = 0;
    const equityCurve = [0]; // starts at 0 profit
    const equityDates = [sub?.createdAt instanceof Date ? sub.createdAt.toISOString() : String(sub?.createdAt ?? '')];
    for (const p of closed) {
...
export async function getShadowSessionLiveStats(userId, sessionId) {
    ...
    const positions = await db.select().from(botPositions)
        .where(and(eq(botPositions.shadowSessionId, sessionId), eq(botPositions.userId, userId)))
        .orderBy(asc(botPositions.openedAt));
    const open = positions.filter(p => p.status === 'open');
    const closed = positions.filter(p => p.status === 'closed');
    ...
    let cumPnl = 0;
    const equityCurve = [0]; // start at 0 (no profit at start)
    const equityDates = [session.startedAt instanceof Date ? session.startedAt.toISOString() : String(session.startedAt ?? '')];
    for (const p of closed) {
    ...
    const dailyPerf = session.dailyPerformance ?? {};
...
function buildMonthlyReturns(dailyPerf, startingBalance) {
    const monthlyPnl = {};
...
// ─── Bot Feed Stats ────────────────────────────────────────────────────────
export async function getBotFeedStats(userId, botId, mode) {
    ...
    const closedChron = [...closedPositions].sort((a, b) => new Date(a.closedAt ?? 0).getTime() - new Date(b.closedAt ?? 0).getTime());
    ...
    let ecCum = 0;
    const equityCurveData = [0];
    const equityDatesData = [];
    for (const p of closedChron) {
...
export async function setBotAvatar(userId, botId, avatarUrl) {
    const [existing] = await db
...
export async function addFavorite(userId, botId) {
...
export async function removeFavorite(userId, botId) {
    await db
        .delete(botFavorites)
        .where(and(eq(botFavorites.userId, userId), eq(botFavorites.botId, botId)));
    return { favorited: false, botId };
}
export async function isFavorited(userId, botId) {
    const [row] = await db
