...
import { eq, and, or, desc, sql, ilike, count, inArray } from 'drizzle-orm';
import { NotFoundError } from '../../lib/errors.js';
import { paginate, paginatedResponse } from '../../lib/pagination.js';
export async function listBots(filters) {
    const { page, limit } = filters;
    const paginationParams = { page, limit };
    const { limit: take, offset } = paginate(paginationParams);
    const conditions = [eq(bots.isPublished, true)];
    if (filters.category) {
        conditions.push(eq(bots.category, filters.category));
    }
    if (filters.risk_level) {
        conditions.push(eq(bots.riskLevel, filters.risk_level));
    }
    if (filters.search) {
        conditions.push(or(ilike(bots.name, `%${filters.search}%`), ilike(bots.strategy, `%${filters.search}%`)));
    }
...
export async function getBotById(botId, userId) {
    ...
    const formattedReviews = botReviews.map((r) => ({
        ...
        userInitials: (r.userName ?? 'A').split(' ').map((w) => w[0]).join('').substring(0, 2).toUpperCase(),
    ...
    if (subIds.length > 0) {
        ...
        recentTrades = await db
            ...
            .from(trades)
            .where(and(inArray(trades.botSubscriptionId, subIdList), eq(trades.isPaper, false), // never expose shadow/paper trades to other users
  ...
  `);
    ...
    const subscriberCount = await db.execute(sql `
    SELECT
  ...
  `);
    ...
    if (userId) {
        ...
        `);
                ...
                userReturn30d = ((pnl30d / allocAmount) * 100).toFixed(2);
            }
        }
    }
    return {
