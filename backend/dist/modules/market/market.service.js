...
async function fetchKrakenCandles(symbol, tf, limit) {
    ...
    if (!pairKey)
        throw new Error('Kraken: no pair data');
...
async function fetchBinanceCandles(symbol, tf, limit) {
...
async function fetchCoinbaseCandles(symbol, tf, limit) {
    ...
    if (!Array.isArray(json.candles) || json.candles.length === 0)
        throw new Error('Coinbase: no candles');
    return json.candles.slice(0, limit).reverse().map((c) => ({
...
async function fetchCoinGeckoCandles(symbol, _tf, limit) {
    ...
    const candles = json.slice(-limit).map((c) => ({
        timestamp: Number(c[0]),
...
async function fetchCryptoCandles(symbol, tf, limit, exchange) {
    const attempts = [];
...
// ─── Live Price ───────────────────────────────────────────────────────────────
async function fetchLivePriceKraken(symbol) {
    try {
...
async function fetchLivePriceCoinbase(symbol) {
    try {
...
async function fetchLivePriceKuCoin(symbol) {
    try {
...
async function fetchLivePriceCoinGecko(symbol) {
    try {
        const geckoId = getCoinGeckoId(symbol);
        if (!geckoId)
            return null;
        ...
        const res = await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${geckoId}&vs_currencies=usd`, { signal: AbortSignal.timeout(5000) });
        if (!res.ok)
            return null;
        const json = await res.json();
...
export async function getLivePrice(symbol, exchange) {
    ...
    for (const ex of tryOrder) {
        ...
        switch (ex) {
            case 'kraken':
                price = await fetchLivePriceKraken(symbol);
                source = 'Kraken';
                break;
            case 'binance':
                price = await fetchLivePriceBinance(symbol);
                source = 'Binance';
                break;
            case 'coinbase':
                price = await fetchLivePriceCoinbase(symbol);
                source = 'Coinbase';
                break;
            case 'kucoin':
                price = await fetchLivePriceKuCoin(symbol);
                source = 'KuCoin';
                break;
            case 'coingecko':
                price = await fetchLivePriceCoinGecko(symbol);
                source = 'CoinGecko';
                break;
        }
        if (price && price > 0)
