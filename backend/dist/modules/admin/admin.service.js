...
// ---- Bots ----
export async function listBots(page, limit, status) {
    const params = { page, limit };
    const { limit: take, offset } = paginate(params);
    const conditions = [];
    if (status) {
...
export async function approveBot(botId) {
    const [updated] = await db
        .update(bots)
...
export async function rejectBot(botId, reason) {
    const [updated] = await db
        .update(bots)
...
export async function suspendBot(botId) {
    const [updated] = await db
        .update(bots)
...
// ---- Subscriptions ----
export async function listSubscriptions(page, limit) {
    const params = { page, limit };
    const { limit: take, offset } = paginate(params);
    const [totalResult] = await db
        .select({ count: count() })
...
export async function listExchangeConnections(page, limit) {
    const params = { page, limit };
    const { limit: take, offset } = paginate(params);
    const [totalResult] = await db
        .select({ count: count() })
...
export async function getTradesAnalytics() {
    const [totals] = await db
        .select({
        totalTrades: count(),
        buyCount: sql `count(*) filter (where ${trades.side} = 'BUY')`,
        sellCount: sql `count(*) filter (where ${trades.side} = 'SELL')`,
        paperCount: sql `count(*) filter (where ${trades.isPaper} = true)`,
        liveCount: sql `count(*) filter (where ${trades.isPaper} = false)`,
    })
        .from(trades);
    return totals;
}
export async function getUsersAnalytics() {
    const [totals] = await db
        .select({
        totalUsers: count(),
        activeUsers: sql `count(*) filter (where ${users.isActive} = true)`,
        adminCount: sql `count(*) filter (where ${users.role} = 'admin')`,
        creatorCount: sql `count(*) filter (where ${users.role} = 'creator')`,
    })
        .from(users);
...
export async function grantSubscription(userId, planTier, durationDays) {
    // Find the plan by tier
    let [plan] = await db.select().from(subscriptionPlans)
        .where(eq(subscriptionPlans.tier, planTier)).limit(1);
    if (!plan) {
...
export async function updateSettings(_data) {
...
// ---- Bot Reactivation ----
export async function reactivateBot(botId) {
    const [bot] = await db
        .update(bots)
        .set({ status: 'approved', isPublished: true, updatedAt: new Date() })
        .where(eq(bots.id, botId))
...
export async function listTrades(page, limit, userId, botId) {
    const params = { page, limit };
    const { limit: take, offset } = paginate(params);
    const conditions = [];
    if (userId) {
...
// ---- Chat History ----
export async function listChats(page, limit, userId) {
    const params = { page, limit };
    const { limit: take, offset } = paginate(params);
    const conditions = [];
    if (userId) {
...
// ---- Shadow Sessions ----
export async function listShadowSessions(page, limit) {
    const params = { page, limit };
    const { limit: take, offset } = paginate(params);
    const [totalResult] = await db
        .select({ count: count() })
