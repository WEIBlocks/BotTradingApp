...
// ─── Get Available Bots ─────────────────────────────────────────────────────
export async function getAvailableBots(userId, search, page = 1, pageSize = 20, assetClass) {
    ...
    const assetClassCondition = assetClass && assetClass !== 'all'
        ? assetClass === 'crypto'
            ? sql `(${bots.category}::text NOT IN ('Stocks'))`
            : sql `(${bots.category}::text = 'Stocks')`
        : null;
    const conditions = [baseCondition, searchCondition, assetClassCondition].filter((c) => c !== null);
...
// ─── Create Session ─────────────────────────────────────────────────────────
export async function createSession(userId, botIds, durationSeconds = 180, mode = 'shadow', virtualBalance = 10000, cryptoBalance, stockBalance, minOrderValue, unlimited = false) {
    // ── Basic validation ──
    if (botIds.length < 2)
  ...
  `);
    ...
    const botRows = await Promise.all(botIds.map(async (botId) => {
        ...
        if (!bot)
            throw new NotFoundError(`Bot ${botId.slice(0, 8)}`);
    ...
    // ── Balance handling ──
    let finalCryptoBalance;
    let finalStockBalance;
    let totalVirtualBalance;
    if (mode === 'live') {
        ...
        if (hasCrypto) {
            ...
            if (wantedCrypto < 10)
                throw new AppError(400, 'Minimum $10 crypto balance for live arena.');
        ...
        if (hasStocks) {
            ...
            if (wantedStock < 10)
                throw new AppError(400, 'Minimum $10 stock balance for live arena.');
...
export async function getSession(sessionId, userId) {
    ...
    const enrichedGladiators = await Promise.all(gladiators.map(async (g) => {
        ...
        const positions = await db.select().from(botPositions)
            .where(and(eq(botPositions.botId, g.botId), eq(botPositions.userId, userId), eq(botPositions.isPaper, !isLiveMode), eq(botPositions.shadowSessionId, g.id)))
...
// ─── Get Results ────────────────────────────────────────────────────────────
export async function getSessionResults(sessionId, userId) {
    const [session] = await db.select().from(arenaSessions)
        .where(and(eq(arenaSessions.id, sessionId), eq(arenaSessions.userId, userId)))
        .limit(1);
    if (!session)
    ...
    const enriched = await Promise.all(gladiators.map(async (g) => {
        ...
        const dbPositions = await db.select().from(botPositions)
            .where(and(eq(botPositions.botId, g.botId), eq(botPositions.userId, userId), eq(botPositions.isPaper, !isLiveSession), eq(botPositions.shadowSessionId, g.id)))
        ...
        const logBuys = log.filter((d) => d.action === 'BUY').length;
        const logSells = log.filter((d) => d.action === 'SELL').length;
        const logHolds = log.filter((d) => d.action === 'HOLD').length;
        return {
            ...g,
            assetClass,
            startingAlloc,
