...
import { uploadFileBodySchema, uploadsParamsSchema, startTrainingBodySchema, dataResponseSchema, } from './training.schema.js';
...
export async function trainingRoutes(app) {
    ...
    zApp.post('/upload-file', {
        ...
        // Save to DB
        const upload = await trainingService.uploadFile(request.user.userId, botId, fileType, safeName || 'Untitled', fileUrl, fileSize);
        return { data: upload };
    });
    ...
    zApp.post('/upload', {
        ...
        const upload = await trainingService.uploadFile(request.user.userId, botId, type, name, fileUrl, fileSize);
        return { data: upload };
    });
    // GET /uploads/:botId
    ...
    zApp.get('/summary/:botId', {
        ...
        const { botId } = request.params;
        const summary = await trainingService.getTrainingSummary(request.user.userId, botId);
        return { data: summary };
    });
    ...
    zApp.get('/upload/:uploadId', {
        ...
        const upload = await trainingService.getUploadById(request.user.userId, uploadId);
        if (!upload) {
