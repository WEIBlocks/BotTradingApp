...
import { getTopAssets, getStockQuotes, extractStockSymbols, extractCryptoPairs, resolveTokenPrice, searchDexScreener } from '../../lib/market-scanner.js';
    ...
    if (!marketContext) {
        try {
            ...
            if (cryptoSymbols.length > 0) {
                const priceData = await Promise.all(cryptoSymbols.slice(0, 6).map(sym => resolveTokenPrice(sym)));
                const valid = priceData.filter(Boolean);
                if (valid.length > 0) {
                    const lines = valid.map(d => {
                        ...
                        if (d.volume24h)
                            line += ` | Vol: $${(d.volume24h / 1_000_000).toFixed(1)}M`;
                        if (d.source === 'dexscreener') {
                            if (d.chain)
                                line += ` | Chain: ${d.chain}`;
                            if (d.liquidity)
                                line += ` | Liquidity: $${(d.liquidity / 1_000).toFixed(0)}K`;
                            if (d.marketCap && d.marketCap > 0)
                                line += ` | MCap: $${(d.marketCap / 1_000_000).toFixed(1)}M`;
            ...
            if (stockSymbols.length > 0) {
                ...
                if (quotes.length > 0) {
                    const stockLines = quotes.map(q => `${q.symbol}: $${q.price.toFixed(2)} | ${q.changePercent >= 0 ? '+' : ''}${q.changePercent.toFixed(2)}% | High: $${q.high.toFixed(2)} | Low: $${q.low.toFixed(2)} | Vol: ${(q.volume / 1_000_000).toFixed(1)}M`).join('\n');
            ...
            if (!marketContext && /price|how much|trading|worth|token|coin|launch|listed|chart|dex|pump|gem/i.test(message)) {
                ...
                if (quotedMatch)
                    candidates.push(...quotedMatch.map(s => s.replace(/["']/g, '').trim()));
