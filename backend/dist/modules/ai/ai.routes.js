...
function checkRateLimit(userId, maxRequests = 20, windowMs = 60_000) {
    const now = Date.now();
...
export async function aiRoutes(app) {
    ...
    app.post('/chat/stream', {
        ...
        if (!checkRateLimit(request.user.userId)) {
            return reply.status(429).send({ message: 'Too many requests. You can send up to 20 messages per minute.' });
        }
        ...
        try {
            const gen = aiService.chatStream(request.user.userId, message, conversationId, attachmentUrl, botId);
    ...
    zApp.post('/youtube/learn', {
        ...
        const info = await getVideoInfo(url);
        if (!info)
            return { error: 'Could not fetch video info. Please check the URL and try again.' };
        ...
        let classifyReason = '';
        try {
            const classifyReply = await llmChat([{ role: 'user', content: `Analyze this YouTube video and determine if it is specifically about crypto, stocks, trading, or investing.\n\n${classifyContext}\n\nRespond with EXACTLY one line: "YES: <one sentence reason>" or "NO: <one sentence reason>". Nothing else.` }], { maxTokens: 60, temperature: 0 });
