...
async function verifyGooglePlay(data) {
    ...
    try {
        if (data.type === 'subscription') {
            ...
            const state = r.subscriptionState ?? '';
...
async function verifyApple(data) {
    ...
    try {
        ...
        if (data.type === 'subscription') {
            const receipts = result.latest_receipt_info ?? [];
            const matching = receipts
                .filter((r) => r.product_id === data.productId)
            ...
            const match = inApp.find((r) => r.product_id === data.productId);
...
export async function verifyReceipt(userId, data) {
