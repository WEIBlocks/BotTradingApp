...
function safeInArray(column, ids) {
    if (ids.length === 0)
        return sql `false`;
    if (ids.length === 1)
...
import { bots, botStatistics, botSubscriptions, shadowSessions, reviews, creatorEarnings } from '../../db/schema/bots.js';
...
export async function getStats(userId) {
    ...
    const [subCount] = await db
        ...
        .where(and(eq(bots.creatorId, userId), eq(botSubscriptions.status, 'active')));
    ...
    const [earningsResult] = await db
        .select({
        total: sql `coalesce(sum(${creatorEarnings.creatorEarning}::numeric), 0)`,
    })
    ...
    const [paymentResult] = await db
        ...
        .from(payments)
        .where(and(eq(payments.status, 'succeeded'), eq(payments.type, 'bot_purchase')));
...
export async function getMonthlyRevenue(userId, months = 6) {
...
export async function getCreatorBots(userId) {
    const rows = await db
        .select({
        id: bots.id,
        name: bots.name,
    ...
    const enriched = await Promise.all(rows.map(async (bot) => {
        const posQuery = await db.execute(sql `
      SELECT
    ...
    `);
        const subQuery = await db.execute(sql `
    ...
    `);
        ...
        const decStats = await db.execute(sql `
      SELECT
        count(*)::int as total,
    ...
    `);
        ...
        const [earning] = await db.select({ total: sql `COALESCE(sum(creator_earning::numeric), 0)` })
        ...
        const perUserStats = await db.execute(sql `
      SELECT
        u.id as user_id,
        u.name as user_name,
    ...
    `);
        const userBreakdown = (perUserStats || []).map((r) => ({
            userId: r.user_id,
...
export async function publishBot(userId, botId) {
 ...
 * normal publish flow.
 */
export async function unpublishBot(userId, botId) {
    const [updated] = await db
        .update(bots)
...
export async function getEarningsSummary(userId) {
    ...
    const [totals] = await db
        .select({
        totalEarnings: sql `coalesce(sum(${creatorEarnings.creatorEarning}::numeric), 0)`,
        totalPlatformFees: sql `coalesce(sum(${creatorEarnings.platformFee}::numeric), 0)`,
        totalSubscriberProfits: sql `coalesce(sum(${creatorEarnings.subscriberProfit}::numeric), 0)`,
        count: count(),
    })
        .from(creatorEarnings)
        .where(eq(creatorEarnings.creatorId, userId));
    ...
    const [pending] = await db
        .select({
        amount: sql `coalesce(sum(${creatorEarnings.creatorEarning}::numeric), 0)`,
    })
        .from(creatorEarnings)
        .where(and(eq(creatorEarnings.creatorId, userId), eq(creatorEarnings.status, 'pending')));
    ...
    const botEarnings = await db
        ...
        botName: bots.name,
        totalEarning: sql `sum(${creatorEarnings.creatorEarning}::numeric)`,
        totalSubscriberProfit: sql `sum(${creatorEarnings.subscriberProfit}::numeric)`,
        transactions: count(),
    })
        .from(creatorEarnings)
...
export async function getEarningsProjection(userId) {
    ...
    const botsWithSubs = await db
        ...
        .from(bots)
        .leftJoin(botStatistics, eq(bots.id, botStatistics.botId))
        .where(and(eq(bots.creatorId, userId), eq(bots.isPublished, true)));
...
export async function getEngagementMetrics(creatorId, days = 30) {
    ...
    const analyticsRows = await db
        ...
        .where(and(eq(creatorAnalytics.creatorId, creatorId), gte(creatorAnalytics.date, sql `now() - (${days} || ' days')::interval`)))
    ...
    const [recentSubs] = await db
        .select({ count: count() })
        .from(botSubscriptions)
        .where(and(safeInArray(botSubscriptions.botId, botIds), gte(botSubscriptions.createdAt, sql `now() - (${days} || ' days')::interval`)));
    const [stoppedSubs] = await db
        .select({ count: count() })
        .from(botSubscriptions)
        .where(and(safeInArray(botSubscriptions.botId, botIds), eq(botSubscriptions.status, 'stopped'), gte(botSubscriptions.updatedAt, sql `now() - (${days} || ' days')::interval`)));
    const [revenueResult] = await db
        .select({ total: sql `coalesce(sum(creator_earning::numeric), 0)` })
        .from(creatorEarnings)
        .where(and(eq(creatorEarnings.creatorId, creatorId), gte(creatorEarnings.createdAt, sql `now() - (${days} || ' days')::interval`)));
...
// ─── User Profitability ───────────────────────────────────────────────────
export async function getUserProfitability(creatorId) {
    const creatorBotIds = await db
        .select({ id: bots.id })
        .from(bots)
        .where(eq(bots.creatorId, creatorId));
...
export async function createExperiment(creatorId, data) {
    ...
    const [existing] = await db
        ...
        .from(botExperiments)
        .where(and(eq(botExperiments.botId, data.botId), eq(botExperiments.status, 'running')))
        .limit(1);
    if (existing)
        throw new ConflictError('An experiment is already running for this bot');
...
export async function getExperiments(creatorId) {
    return db
        .select({
...
export async function getExperimentResults(experimentId, creatorId) {
    const [exp] = await db
        .select()
        .from(botExperiments)
...
export async function stopExperiment(experimentId, creatorId) {
    const [exp] = await db
...
export async function getBotPatternAnalysis(botId, creatorId) {
    // Verify ownership
    const [bot] = await db
    ...
    const [existing] = await db
        ...
        .where(and(eq(botPatternAnalysis.botId, botId), gte(botPatternAnalysis.analysisDate, sql `now() - interval '24 hours'`)))
...
Respond in JSON only:
{
  ...
  "suggestedImprovements": [{"title": "...", "description": "...", "impact": "high|medium|low"}]
    ...
    try {
        const aiResponse = await llmChat([{ role: 'user', content: analysisPrompt }], { system: 'You are a quantitative trading analyst. Respond only with valid JSON.' });
...
// ─── Churn Analysis ────────────────────────────────────────────────────────
export async function getChurnAnalysis(creatorId) {
    const creatorBotIds = await db
  ...
  `);
    ...
    const [atRisk] = await db
        .select({ count: count() })
        .from(botSubscriptions)
        .where(and(safeInArray(botSubscriptions.botId, botIds), eq(botSubscriptions.status, 'active'), lte(botSubscriptions.updatedAt, sql `now() - interval '14 days'`)));
    return {
        overallChurnRate: Number(overallChurnRate.toFixed(1)),
        activeUsers: activeCount,
        totalUsers: totalCount,
        churnedUsers: stoppedCount,
        monthlyChurn: monthlyChurnResult,
        botChurn: botChurn.sort((a, b) => b.churnRate - a.churnRate),
        atRiskUsers: atRisk?.count ?? 0,
    };
}
// ─── Enhanced Revenue Projection ───────────────────────────────────────────
export async function getEnhancedRevenueProjection(creatorId) {
...
// ─── Marketing Funnel ──────────────────────────────────────────────────────
export async function getMarketingMetrics(creatorId) {
    const creatorBotIds = await db
    ...
    const [purchased] = await db
        .select({ count: sql `count(distinct ${botSubscriptions.userId})` })
        .from(botSubscriptions)
        .where(safeInArray(botSubscriptions.botId, botIds));
    ...
    const [reviewers] = await db
        .select({ count: sql `count(distinct ${reviews.userId})` })
        .from(reviews)
...
export async function getBotSubscriberDetails(botId, creatorId) {
...
export async function getBotTradeSummary(botId, creatorId) {
    const [bot] = await db.select().from(bots).where(and(eq(bots.id, botId), eq(bots.creatorId, creatorId)));
    if (!bot)
        throw new NotFoundError('Bot');
    // Get all decisions for this bot
    const { botDecisions } = await import('../../db/schema/decisions');
    const decisionStats = await db.execute(sql `
    SELECT
  ...
  `);
    const ds = decisionStats?.[0] || {};
    // Get position summary
    const positionStats = await db.execute(sql `
    SELECT
      count(*)::int as total,
  ...
  `);
    ...
    const recentTrades = await db
        ...
        .where(sql `${trades.botSubscriptionId} IN (SELECT id FROM bot_subscriptions WHERE bot_id = ${botId})
