...
// ─── Binance Testnet API ─────────────────────────────────────────────────────
async function checkBinanceTestnet(apiKey, apiSecret) {
    ...
    const nonZeroBalances = (acct.balances || []).filter((b) => parseFloat(b.free) > 0 || parseFloat(b.locked) > 0);
    console.log(`  ✅ Account connected. Non-zero balances:`);
    nonZeroBalances.slice(0, 10).forEach((b) => {
...
async function main() {
    ...
    const binanceConn = conns.find(c => c.provider.toLowerCase().includes('binance') && c.sandbox === true);
    if (binanceConn) {
