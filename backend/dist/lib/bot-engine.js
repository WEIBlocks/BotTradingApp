...
// ─── Caches ─────────────────────────────────────────────────────────────────
const priceHistoryBuffer = new Map();
const indicatorCache = new Map();
const rulesCache = new Map();
const lastDecisionCache = new Map();
const aiCallCount = new Map();
...
function addPrice(key, price) {
    ...
    hist.push(price);
    if (hist.length > MAX_PRICE_HISTORY)
        hist.shift();
    return hist;
}
...
function checkAIRateLimit(botId, symbol) {
...
export const DEFAULT_COMPOUNDING = {
    enabled: false,
    reinvestmentRate: 50,
...
export async function applyCompounding(opts) {
    ...
    if (settings.compoundFrequency !== 'each_trade' && settings.lastCompoundAt) {
        ...
        if (settings.compoundFrequency === 'daily' && hoursSince < 24)
            return null;
        if (settings.compoundFrequency === 'weekly' && hoursSince < 168)
            return null;
    }
    // Check minimum profit threshold
    if (pnl < settings.minProfitThresholdUSD)
        return null;
    ...
    await db.update(botSubscriptions).set({
        ...
        compoundingSettings: updatedSettings,
        updatedAt: new Date(),
...
async function releaseLock(key) {
    try {
        await redisConnection.del(`lock:${key}`);
    }
    catch { }
}
// ─── Strategy Detection ─────────────────────────────────────────────────────
...
Generate trading rules JSON.`;
    try {
        const response = await engineLLMChat([{ role: 'user', content: prompt }], { system: RULE_GENERATION_SYSTEM, maxTokens: 1024, temperature: 0.1, cacheSystem: true });
        ...
        if (stopLoss)
            rules.stopLossPercent = stopLoss;
        if (takeProfit)
            rules.takeProfitPercent = takeProfit;
        if (maxPosition)
            rules.maxPositionPercent = maxPosition;
        if (!rules.cooldownMinutes)
            rules.cooldownMinutes = 5;
        if (!rules.entryConditions?.length)
            rules.entryConditions = [{ indicator: 'rsi', operator: '<', value: 35, weight: 0.7 }];
        if (!rules.exitConditions?.length)
            rules.exitConditions = [{ indicator: 'rsi', operator: '>', value: 65, weight: 0.7 }];
        // Apply risk level multiplier
        applyRiskLevel(rules, riskLevel);
        return rules;
    }
    catch (err) {
        console.warn('[BotEngine] Rule generation failed, using defaults:', err.message);
        return getDefaultRules(strategy, riskLevel, stopLoss, takeProfit, maxPosition);
    }
}
function applyRiskLevel(rules, riskLevel) {
    const multipliers = {
...
function getDefaultRules(strategy, riskLevel, sl, tp, mp) {
...
function getIndicatorValue(indicator, snap) {
    switch (indicator) {
...
function evaluateCondition(cond, snap, prevSnap) {
...
function evaluateRules(conditions, snap, prevSnap) {
    if (!conditions.length)
        return { matched: false, score: 0, matchedConditions: [] };
...
const positionMonitorCache = new Map(); // positionId → last call timestamp
...
async function monitorOpenPosition(opts) {
    ...
    if (Date.now() - lastCall < MONITOR_INTERVAL_MS)
        return null;
    ...
    try {
        ...
        if (peakStr)
            peakPnl = Math.max(opts.positionPnlPct, parseFloat(peakStr));
    }
    catch { }
...
Decide what to do with this open position RIGHT NOW.`;
    try {
        const response = await engineLLMChat([{ role: 'user', content: prompt }], { system: POSITION_MONITOR_SYSTEM, maxTokens: 350, temperature: 0.15, cacheSystem: true });
        ...
        if (!jm)
            throw new Error('No JSON in monitor response');
        ...
        return {
            action: (validActions.includes(result.action) ? result.action : 'HOLD'),
        ...
        console.warn('[BotEngine] Position monitor AI failed:', err.message);
...
async function writePostTradeNote(botId, symbol, note) {
    try {
        ...
        console.warn('[BotEngine] writePostTradeNote failed:', err.message);
    }
}
// ─── AI Decision ────────────────────────────────────────────────────────────
...
${recentDecisions ? `Recent: ${recentDecisions.slice(0, 300)}` : ''}`;
    try {
        const response = await engineLLMChat([{ role: 'user', content: prompt }], { system: AI_DECISION_SYSTEM, maxTokens: 400, temperature: 0.2, cacheSystem: true });
        let text = response.text;
        const cb = text.match(/```(?:json)?\s*([\s\S]*?)```/);
        if (cb)
            text = cb[1].trim();
        ...
        console.warn('[BotEngine] AI decision failed:', err.message);
        return { action: 'HOLD', confidence: 0, sizePercent: 0, reasoning: 'AI unavailable — holding', tokens: 0 };
    }
}
// ─── Main Engine ────────────────────────────────────────────────────────────
    ...
    try {
        ...
        if (dailyLossLimit > 0) {
            ...
            if (dailyLoss >= dailyLossLimit) {
                const pauseDecision = { action: 'HOLD', confidence: 100, reasoning: `Daily loss limit reached (${dailyLoss.toFixed(2)}% / ${dailyLossLimit}%). Trading paused until tomorrow.`, indicators: {}, aiCalled: false, tokensCost: 0, price: priceData.price, symbol };
        ...
        const regime = detectMarketRegime(prices, priceData.price, priceData.high24h, priceData.low24h, priceData.change24h ?? 0);
        snap.regime = regime;
        ...
        if (existingPos && positionPnlPct !== null && !isRuleOnly && (opts.aiMode === 'hybrid' || opts.aiMode === 'full_ai')) {
            try {
                ...
                console.warn('[BotEngine] Position monitor error:', monitorErr.message);
            }
        }
        ...
        if (existingPos && positionPnlPct !== null) {
            ...
            try {
                ...
                console.warn('[BotEngine] Trailing stop Redis error:', err.message);
            }
        }
        ...
        if (!isFullAI && lastDec && lastDec.action !== 'HOLD' && Date.now() - lastDec.time < rules.cooldownMinutes * 60000) {
            const cooldownDecision = { action: 'HOLD', confidence: 50, reasoning: `Cooldown active (${rules.cooldownMinutes}min). Last: ${lastDec.action}`, indicators: formatIndicators(snap), aiCalled: false, tokensCost: 0, price: priceData.price, symbol };
        ...
        const entryResult = isFullAI
            ? { matched: false, score: 0, matchedConditions: [] }
            : evaluateRules(rules.entryConditions, snap, prevSnap);
        const exitResult = isFullAI
            ? { matched: false, score: 0, matchedConditions: [] }
            : (existingPos ? evaluateRules(rules.exitConditions, snap, prevSnap) : { matched: false, score: 0, matchedConditions: [] });
        // 8. Decision logic
        ...
        const freqScoreMap = { conservative: 0.5, balanced: 0.2, aggressive: 0.1, max: 0.05 };
        ...
        if (shouldCallAI) {
            ...
            const extractIndicator = (reasoning) => {
                if (!reasoning)
                    return 'unknown';
            ...
            if (finalAction === 'BUY' && existingPos)
                finalAction = 'HOLD';
            if (finalAction === 'BUY' && tradeDirection === 'sell')
                finalAction = 'HOLD';
            if (finalAction === 'SELL' && tradeDirection === 'buy')
                finalAction = 'HOLD';
            if (finalAction === 'SELL' && !existingPos)
                finalAction = 'HOLD';
            const confThreshold = opts.aiConfidenceThreshold ?? 60;
            if (finalAction === 'BUY' && aiResult.confidence < confThreshold)
                finalAction = 'HOLD';
            if (finalAction === 'SELL' && aiResult.confidence < confThreshold)
                finalAction = 'HOLD';
            if (finalAction !== 'HOLD') {
            ...
            const holdDecision = { action: 'HOLD', confidence: 50, reasoning: reason, indicators: formatIndicators(snap), aiCalled: false, tokensCost: 0, price: priceData.price, symbol };
            holdCountCache.set(cacheKey, (holdCountCache.get(cacheKey) ?? 0) + 1);
            ...
            if (closed) {
                ...
                if (opts.subscriptionId && closed.pnlNum > 0) {
                    try {
                        ...
                        if (sub) {
                            const cs = (sub.compoundingSettings ?? DEFAULT_COMPOUNDING);
                        ...
                        console.warn('[BotEngine] Compounding check failed:', err.message);
                    }
                }
            }
        }
...
async function logDecision(decision, opts) {
    ...
    try {
        ...
        console.error('[BotEngine] Failed to log decision:', err.message);
    }
    ...
    try {
        ...
        console.error('[BotEngine] Redis publish failed:', err.message);
    }
}
// ─── Execute Live Trade ─────────────────────────────────────────────────────
export async function executeLiveTrade(decision, userId, botId, subscriptionId, exchangeConnId, orderType = 'market') {
    try {
        ...
        if (conn.status === 'disconnected' || conn.status === 'error')
            return { success: false, error: 'Exchange disconnected' };
 ...
 */
export function applyTrainerConfigToRulesCache(botId, configChanges) {
    const key = `rules:${botId}`;
    const cached = rulesCache.get(key);
    if (!cached)
        return; // cache already empty — next cycle will regenerate fresh
    const updated = { ...cached.rules };
    if (configChanges.stopLoss != null)
        updated.stopLossPercent = Number(configChanges.stopLoss);
    if (configChanges.takeProfit != null)
        updated.takeProfitPercent = Number(configChanges.takeProfit);
    if (configChanges.maxPositionPct != null)
        updated.maxPositionPercent = Number(configChanges.maxPositionPct);
    // Tighter cooldown for aggressive mode, wider for conservative
    if (configChanges.tradingFrequency != null) {
        const freqMap = { conservative: 60, balanced: 30, aggressive: 10, max: 5 };
...
export async function summarizeSessionLearnings(botId) {
    ...
    const extractIndicators = (reasoning) => {
        if (!reasoning)
            return [];
        const lower = reasoning.toLowerCase();
    ...
    try {
        ...
        if (notes && notes.length > 0)
            postTradeContext = ` Recent trade outcomes: ${notes.join(' | ')}.`;
    }
    catch { }
...
function formatIndicators(snap) {
    return {
