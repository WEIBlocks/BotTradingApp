...
async function fetchAlpacaQuotes(symbols) {
    ...
    try {
        ...
        if (!barsRes.ok)
            throw new Error(`Alpaca bars ${barsRes.status}`);
...
export function extractStockTickers(message) {
...
export function extractCryptoPairs(message) {
...
export function extractStockSymbols(message) {
    const lower = message.toLowerCase();
    const upper = message.toUpperCase();
    const found = new Set();
...
async function fetchOHLCVCloses(symbol, limit = 50) {
    const cached = ohlcvCache.get(symbol);
    if (cached && Date.now() - cached.at < OHLCV_MEM_TTL)
    ...
    try {
        ...
        const closes = candles.map((c) => parseFloat(c[2])).filter(Boolean).reverse();
 ...
 */
async function scoreSymbol(symbol, ticker) {
    ...
    if (rsi !== null) {
        if (rsi >= 25 && rsi <= 45)
            rsiScore = 100; // ideal oversold bounce
        else if (rsi > 45 && rsi <= 60)
            rsiScore = 70; // healthy uptrend
        else if (rsi > 60 && rsi <= 70)
            rsiScore = 40; // extended, caution
        else if (rsi > 70)
            rsiScore = 10; // overbought — avoid
        else
            rsiScore = 30; // deeply oversold <25 — risky
    }
...
export async function searchDexScreener(query) {
    ...
    if (cached && Date.now() - cached.at < DEX_CACHE_TTL)
        return cached.data;
    try {
 ...
 */
export async function getDexPrice(symbolOrName) {
