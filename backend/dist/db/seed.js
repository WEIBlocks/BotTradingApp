...
import { bots, botVersions, botStatistics, botSubscriptions, shadowSessions, reviews, } from "./schema/bots.js";
import { trades } from "./schema/trades.js";
import { subscriptionPlans, userSubscriptions, } from "./schema/subscriptions.js";
import { paymentMethods, payments } from "./schema/payments.js";
import { notifications, notificationSettings, } from "./schema/notifications.js";
import { arenaSessions, arenaGladiators } from "./schema/arena.js";
import { trainingUploads, activityLog } from "./schema/training.js";
import { exchangeConnections, exchangeAssets, } from "./schema/exchanges.js";
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function daysAgo(n) {
...
async function seed() {
    ...
    const [creator2] = await db
        ...
        .returning();
    console.log(`  Created ${5} users: admin, 2 creators, 2 regular users.\n`);
    // --------------------------------------------------
    // 3. Create bots
    // --------------------------------------------------
    ...
    const botDefs = [
        {
            ...
            description: "Advanced momentum strategy that identifies Bitcoin trend continuations using RSI, MACD, and volume analysis. Optimized for 4H timeframe with dynamic stop-losses.",
            strategy: "Momentum",
            category: "Crypto",
            riskLevel: "Med",
            priceMonthly: "29.99",
        ...
        },
        {
            ...
            description: "Captures small price movements in ETH/USDT using order book analysis and mean reversion on 1m-5m charts. High trade frequency, low risk per trade.",
            strategy: "Scalping",
            category: "Crypto",
            riskLevel: "High",
            priceMonthly: "49.99",
        ...
        },
        {
            ...
            description: "Monitors Solana DeFi protocols for arbitrage and yield farming opportunities. Executes trades across Jupiter, Raydium, and Orca DEXs.",
        ...
        },
        {
            ...
            description: "Grid-based strategy for EUR/USD, GBP/USD, and USD/JPY. Places buy and sell orders at fixed intervals to profit from ranging markets.",
        ...
        },
        {
            ...
            description: "Uses natural language processing to analyze crypto news, social media sentiment, and on-chain data. Generates signals based on sentiment shifts before price action.",
        ...
        },
        {
            ...
            description: "Identifies swing trading opportunities in S&P 500 stocks using weekly support/resistance levels and RSI divergence patterns.",
        ...
        },
        {
            ...
            description: "Monitors Bitcoin dominance and altcoin relative strength to rotate into high-momentum altcoins during alt seasons. Covers top 50 altcoins by market cap.",
            strategy: "Rotation",
            category: "Crypto",
            riskLevel: "Very High",
            priceMonthly: "34.99",
        ...
        },
        {
            ...
            description: "Enhances traditional DCA with volatility-adjusted sizing. Buys more when fear is high and less during euphoria. Supports BTC, ETH, and SOL.",
            strategy: "DCA",
            category: "Crypto",
            riskLevel: "Very Low",
            priceMonthly: "0",
        ...
        },
        {
            ...
            description: "Hedges crypto portfolio volatility by monitoring gold/USD correlation and shifting allocation to XAU/USD and stable forex pairs during high-risk periods.",
            strategy: "Hedging",
            category: "Multi",
            riskLevel: "Low",
            priceMonthly: "29.99",
        ...
        },
        {
            ...
            description: "Monitors consolidation patterns on BTC, ETH, and SOL. Enters breakout trades with tight stop-losses when volume confirms the move. 1H timeframe.",
            strategy: "Breakout",
            category: "Crypto",
            riskLevel: "Med",
            priceMonthly: "34.99",
    ...
    console.log(`  Created 2 payment methods and ${paymentDefs.length} payments.\n`);
    // --------------------------------------------------
    ...
    console.log(`  Created 1 arena session with ${gladiatorDefs.length} gladiators.\n`);
    // --------------------------------------------------
    // Summary
    // --------------------------------------------------
